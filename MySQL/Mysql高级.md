...
*   适合数据量大且查询频率高的表
*   适合经常出现在where子句中的字段
*   要统计的字段建立索引，比如 count(), max()
*   需要排序的字段建立索引，因为索引树自带排序功能
*   需要分组的字段建立索引，因为要分组，得先排序，排序能用上索引会快很多
...
*   字符串类型的列作为条件进行筛选时，一定要加上双引号，否则可能自动类型转换为整型而使得索引失效
...
要避免跨表跨库join 的情况发生，因为它很难处理，有下面集中解决方法
