...
但是索引也很大，不能全部一次性读取，mysql按照页存储数据，按页读取数据，每次读一个页涉及一次磁盘IO，页大小固定，那么如果记录很大，每个页中存储的记录数就少，需要读取的次数就增多，效率就下降
...
###### [全文索引](https://www.cnblogs.com/wxzhe/p/9955534.html)

又称--倒排索引，等同于 elasticsearch 中的倒排索引

实际上是一种统计的方法，先分词，然后根据分词创建分词位置表

| 单词  | 位置     |
| ----- | -------- |
| alice | [1,2,48] |

查询时从分词位置表中查询该单词出现在哪些位置，然后遍历这些位置的记录

全文索引有两种：

![image-20220420184756806](Mysql高级.assets/image-20220420184756806.png)
...
然而在innodb存储引擎的全文索引中，还有另外一个重要的概念FTS index cache（全文索引缓存），其用来提高全文检索的性能。FTS  index cache是一个红黑树结构，其根据（word，  ilist）进行排序。这意味着插入的数据已经更新了对应的表，但是对全文索引的更新可能在分词操作后还在FTS index  cache中，辅助表可能还没有更新。innodb存储引擎会批量对辅助表进行更新，而不是每次插入后更新一次辅助表。当对全文索引进行查询时，辅助表会首先将FTS index cache中对应的word字段合并到辅助表中，然后再进行查询。
...
查询先看缓存，未命中后查看索引（读取索引分为直接能从内存中读到和未从内存读取到而读取磁盘），未命中后访问磁盘读取数据，所以这里可以得知，无缓存无索引的情况下，总是会走磁盘io
...
count(*) 数据库做了优化，取出记录后直接记录加一，效率最高
...
RC时, 仅仅使用 `行锁` , 锁的粒度更小, 能有效提高并发, 但是此时需要手动对幻读问题进行处理 , 且不能再使用 statement 的 binlog , 换成 row 或者 mixed
...
#### [MySQL调优](https://mp.weixin.qq.com/s?__biz=MzU4NzA3MTc5Mg==&mid=2247484508&idx=1&sn=4e81d365409bf32c08e4ea985e3ca593&chksm=fdf0ec03ca876515d59c49f033cf83f72b62fafe356e678b4d162ad3623d31bf60fb6620176f&token=336229290&lang=zh_CN&scene=21#wechat_redirect)

***<span style='color:cyan;'>开发规范</span>***

* 尽量索引覆盖
* 联合索引考虑最左匹配原则, 将区分度最高的提前到左边
* 使用预编译查询
* 使用 union all 而不是 union
* 避免对索引列的表达式计算
* 分页查询使用先查询 id 再查询记录的方式强制使用索引
* 使用 explain 查看索引实际使用情况
* 在代码逻辑中, 开启事务后尽量只做数据库相关操作, 减少事务时间

***<span style='color:cyan;'>隔离级别</span>***

* MySQL默认 rr, 对于高并发场景, 可以切换为 rc, 然后要注意处理主从复制不一致的问题

* 注意 Oracle 中仅仅提供 RC 和 Serializable, 默认 RC

***<span style='color:cyan;'>走了索引还是慢, 考虑数据量大的问题</span>***

* 删除旧数据, 或者旧数据离线存储, 这种方式将不支持后续对古早数据的查询
* 通过 Redis 缓存数据, 注意要考虑不能实时一致性的问题
* 将数据导入 ElasticSearch 等搜索引擎, 然后直接从搜索引擎查询, 监听MySQL 的 binlog, 将变动导出到搜索引擎中
* 考虑记录聚合表, 比如将一个用户一天的订单聚合成一条记录, 对于古早数据, 首先聚合存储到聚合表中, 然后从该表中查询, 显然聚合表的量级将会远远小于原表, 但是会损失记录的细节

***<span style='color:cyan;'>写性能瓶颈</span>***

* 使用数据库的主从结构, 主库负责写操作, 从库负责读操作, 读写分离, 减轻写操作的压力, 该机制通过开启 binlog 实现

***<span style='color:cyan;'>读性能瓶颈--分库分表</span>***

* 分库分表, 数据分布的策略应当基于实际的业务需求来定, 比如该业务中对同一个用户的所有记录查询比较高频, 那么可以根据用户id 进行数据的分布, 将同一个用户的数据放到一个数据库中

* 分库分表主键生成策略, 一般有单独库自增主键, Redis自增键, 雪花算法主键生成服务, 改进式leaf算法

  主键的生成强烈建议递增趋势, 因为插入数据时, 要同时加入索引树, 递增的主键有利于插入索引树的效率

* 数据迁移到新表的步骤, 首先将新表创建, 然后对于接下来的新数据, 旧表和新表各存一份, 同时将旧表原有数据复制到新表, 在某个时刻, 旧表数据和新表数据将会一致, 此时则数据迁移完毕

* 数据迁移到新表时, 可以使用双读策略, 一部分读取流量分配到新表, 一部分读取流量分配到旧表, 待到迁移完毕, 全部流量切换到新表, 相当于灰度上线, 这样能够减轻旧表的压力
...
​		因为MySQL会自动对主键生成 B+树索引, 每当插入记录时, 都需要将新纪录插入到索引树中, 如果是自增id, 那么只需要在最后一个叶子节点(磁盘块) 中加入数据即可, 对比随机主键, 每次插入数据将会在索引树的随机位置进行插入, 这将带来很多树的搜索以及重构操作(磁盘块数据迁移) , 性能上将会极大降低
...
> <span style='color:cyan;'>注: 加行锁的前提是 mysql 选择使用了索引树, 如果 mysql 认为某条查询全表扫描效率比使用索引高, 那么都不会走索引树, 而是直接全表扫描, 使用的就是表锁, 更不用提行锁</span>
...
<img src="Mysql高级.assets/1638193105014.png" alt="1638193105014" style="zoom:80%;" />
...
同时, 因为 mysql 是对索引加锁, 不是对行记录加锁, 对于相同索引的记录, 在加锁时会被同时加锁

<img src="Mysql高级.assets/1638194898617.png" alt="1638194898617" style="zoom: 67%;" />
...
#### [事务ACID](https://mp.weixin.qq.com/s?__biz=MzU4NzA3MTc5Mg==&mid=2247484480&idx=1&sn=3571b89575e8c37c114c9f290b953a1c&chksm=fdf0ec1fca87650913e6673a453d0ba1614341433aa67dd9977fef7231a3d825f7da4e4a132a&token=1651214636&lang=zh_CN&scene=21#wechat_redirect)

<img src="Mysql高级.assets/1638193013447.png" alt="1638193013447" style="zoom: 67%;" />
...
​		<del style='color:red;'>serialize read</del>
