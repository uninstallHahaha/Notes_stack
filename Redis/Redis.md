...
* 数据结构
    ...
    ***hashmap***
    ...
    ​		当第一个数组达到扩容条件时，<span style='color:cyan;'>开始渐进式扩容</span>，即每次查询的同时顺便将数据复制到第二个数组中，直至所有数据被迁移，是为了防止一次性的rehash迁移造成cpu繁忙而停止服务
...
![image-20220421091427371](Redis.assets/image-20220421091427371.png)

![image-20220421091458749](Redis.assets/image-20220421091458749.png)

![image-20220421091513892](Redis.assets/image-20220421091513892.png)

![image-20220421091541878](Redis.assets/image-20220421091541878.png)
...
![image-20220421091601479](Redis.assets/image-20220421091601479.png)
...
2.  <span style='color:lightgreen;'>***删除缓存方案***</span>
    ...
    1.  <del style='color:pink;'>先删除缓存，后更新数据库</del>
        ...
        ​		对于这种方案，存在一种流行的解决方案 <span style='color:cyan;'>***`延迟双删除`***</span>， 也就是，线程A在更新完数据库后，延迟一会，再次删除缓存，那么哪怕在A更新数据库之前，其他线程把旧数据又更新回了缓存，我线程A都会给你删掉，这样一来，就缩短了数据不一致的时长

        ​		那么设置多久的延时呢，至少要长过其他线程从数据库获取到值并且将其设置到缓存的时间，需要根据实际情况来定
...
<span style='color:cyan;'>***考虑第二步异常的情况 - 重试机制***</span>
...
<span style='color:cyan;'>***如果不想写代码向消息队列中添加消息 - 使用中间件监听binlog自动更新消息队列***</span>
...
<span style='color:cyan;'>***那么能否做到强一致***</span>

​		答案是不能的，如果不能做到所有操作严格串行，那么总是可以读到中间状态，这就不是强一致，如果想要保证真正的强一致，那么只能牺牲性能，但是这样就失去了最初引入缓存的初衷-提高性能，所以应当在强一致性和性能之间找到平衡

​		<span style='color:pink;'>***值得一提的是，我们总是可以使用失效时间机制来兜底，保证数据的最终一致性***</span>

<span style='color:cyan;'>***所以结论是，使用 `先更新数据库，再删除缓存` 的方案，然后使用 `消息队列` 或者 `订阅binlog变更` 保证第二步的正确执行，来保证数据的一致性***</span>
