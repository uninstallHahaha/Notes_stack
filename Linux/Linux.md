...
#### [Linux命令-awk](https://www.cnblogs.com/ggjucheng/archive/2013/01/13/2858470.html)

总结来说, 就是读取文本文件, 然后按行分割, 在每一行中按分隔符分割, 最后对每个分割出来的部分执行另外一个命令

1. 输入文件
2. 按照换行分割每一行
3. 按照分隔符(默认空格)将每一行分割为一个个域
4. 对这些域进行正则匹配
5. 最后对得到的结果执行其他命令

`awk [-F  field-separator]  '{pattern action}'  input-file(s)`

* field-separator 指定分隔符, 默认为空格
* pattern 正则表达式, 用 \ 和 \ 包裹, 不需要正则匹配时直接不用写这一部分
* action 对结果要执行的其他命令, 比如 print
* input-file 输入的文件

内置变量

![1636335726878](Linux.assets/1636335726878.png)

![1636335710763](Linux.assets/1636335710763.png)

比如

![1636335744001](Linux.assets/1636335744001.png)

现在要仅仅打印用户名

![1636335766034](Linux.assets/1636335766034.png)
