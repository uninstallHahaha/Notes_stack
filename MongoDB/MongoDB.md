...
为某个字段创建索引，就是用额外的一个文件保存 pos 位置值和 该字段值的对应关系，根据该字段值建立 b-tree 结构索引，首先查询索引文件中指定值对应的 pos 值，然后去集合中对应的位置取得该条记录
...
*   数据不安全写，直接写完内存就返回成功，因此不能保证数据的持久化，需要额外地使用副本节点来数据冗余
*   数据预读到内存，大部分读都是直接内存读，所以很快，对比mysql如果查询不能命中缓存，也不能命中索引，那么就会去硬盘读
*   不用考虑文档之间的关联约束关系，本质就是在操作 json，省去了很多检校过程
*   不用考虑事务（因为不支持事务），省去很多流程
*   天生支持数据分片，数据存储可以很容易扩展到多个节点的存储，对比mysql进行分库分表需要涉及太多的难点
