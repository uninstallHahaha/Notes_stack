...
> 基于pytorch的图像工具集, 包含 常用数据集, 模型, 转换函数
...
`t.unsqueeze(n)` 在维度n的位置添加一个宽度为1的维度

` t.squeeze(n)` 去掉维度n的位置的宽度为1的维度，如果这个位置宽度不是1, 操作无事发生

`t.expend(x,y,z,...)` 将原数据扩展为 shape=x,y,z... 的形状，本质就是拿着原来形状的数据在下一个维度上按照宽度复制

*   注意，假设原shape=2,3，目标维度中，最后两位必须是2,3 

    假设原shape=3，目标维度中，最后一位必须是3

    也就是原维度置底

*   注意， 原维度中宽度为1的维度，可以任意扩张

    假设原shape=2,1,3，目标维度中，最后三维必须是 2,x,3 , x可以是任意值

`torch.max(t[,t,t...])`

*   如果参数是一个t, 那么返回该 t 中最大的一个值
*   如果参数是多个t，那么多个 t 的shape 必须相同，返回的是每个位置上多个 t 中的最大值
