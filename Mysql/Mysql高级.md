...
A：Atomicity 原子性，一个事物中的操作要么全部执行要么全部不执行

I：Isolation 隔离性，指的是控制一个事务对数据的修改在什么时间能够被其他事务看到，对应的概念为隔离级别

C：Consistency 一致性，指的是随着时间的推移，数据总是保持正确的状态，比如转账业务中的余额控制

D：Durability 持久性，指的是一旦执行了事务的submit，之前所做的所有操作都不能丢失，都要保证保存到数据库中
