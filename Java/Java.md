...
###### [快速排序](https://blog.csdn.net/qq_19525389/article/details/81436838)

注意， 极端情况下，快排时间复杂度为 n^2，也就是每次选取的参照点刚好是最小值，例如对一个已经严格递增的序列进行从小到大的排序

快排的优化

1. 当前排序部分长度较短时, 改用插入排序
2. 选择参考值时, 不选择数组第一个或者最后一个, 而是取 第一个, 正中间一个, 最后一个, 然后三数排序取中间值
3. 选择参考值时，不选择数组第一个或者最后一个, 而是随机取一个中间的值，计算时间复杂度期望为 $O(\log n)$
4. 对于参考值的重复元素, 在排序时, 先放到数组头和尾, 完成当次排序后, 将这些重复元素移动到参考元素周围, 然后下次递归直接不用考虑这些重复的部分, 能够减少递归次数
...
​		<span style='color:cyan;'>所以注意：字符串常量池才是 jvm机制中真正会单独分配到固定的内存，并且可以在 jvm 参数中配置其大小</span>
...
6.  在第3步保存的栈调用信息中查找该 16进制进程号，可得知具体是哪个类中哪一行调用
7.  定位源代码，查看代码问题
...
\>\> 带符号右移，正数高位补0, 负数高位补1，也就是右移后结果正负性同原数

\>\>\> 无符号右移，不关心原数正负性，直接右移高位补0

因此，`return ((a>>>31)^(b>>>31))==0;` 来判断两个数符号是否相等
...
* Lombok 注解为 Retention.CLASS 注解, 在编译阶段对加了 @Data 注解的实体类生成代码, 也就是它的运行原理
* spring 之类的框架中, 绝大多数都是 RUNTIME 注解, 即注解保留到代码运行阶段

![1636346717394](Java.assets/1636346717394.png)
...
比如状态监控系统中，后端不同接口负责对不同的消息进行处理，其中部分消息需要同步给前端来进行展示

常规做法就是根据这些接口的实际需求，在各个接口中都加入给前端反馈消息的逻辑，但是这样做显然会造成很多重复的代码逻辑
...
* 题目一: 2G内存, 数据文件8G, 如何对该文件进行<span style='color:cyan;'>排序</span>

  方法: 将8G文件分为多个2G的数据段, 每次加载一个数据段进行排序, 最后使用归并的方法同时读取多个(多路)排好序的数据段并输出到结果文件中

* 题目二: 2G内存, 数据文件8G, 如何找到文件中值的<span style='color:cyan;'>中位数</span>

  方法: 先遍历一遍8G的数据, 记录每个区间值的个数, 然后根据个数算得中位数应当在哪个值区间, 接下来再次遍历8G数据, 将符合值区间的记录保存到内存中, 遍历完成后, 对内存中的数据排序取中位数即可
...
2. 为类中静态成员分配内存, 并且赋值默认值 <img src="Java.assets/1638254615910.png" alt="1638254615910" style="zoom:80%;" />
...
```
分布式的话还是建议构建单独的编号生成系统, 比如使用 leaf 架构 或者 redis的incr方法
...
![image-20220413232222767](Java.assets/image-20220413232222767.png)
...
10亿个无符号数字，范围为 $0 - 2^{32}$ ，存储到一个文件中，5G内存，将排序后结果存到一个文件中 

1.  5G内存不能一次性全部加载，那么利用5G内存每次只排序一部分，使用元素类型为 `[数字：出现次数]` 的堆结构
2.  假设 `数字` 部分占 4 字节，`出现次数` 部分占4字节，那么5G内存能够建立包含 $2^{27}$ 个元素的堆结构
3.  遍历所有10亿数，建立其中最小的 $2^{27}$ 个数字的堆结构，并记录当前到达的最大值，将建立完的堆数字输出到新文件
4.  再次遍历10亿数，以上一次记录的最大值为最小阈值，建立大于这个最小阈值的最小的 $2^{27}$ 个数字的堆结构
5.  一直遍历，直至10亿数都被保存到新文件
...
​		首先计算所有数据hash，对hash取余将所有数据分为多个小文件存储；

​		对每个小文件内的数据计算topk大根堆；

​		使用所有子大根堆的top1，创建综合topk大根堆；

​		每次从综合大根堆中弹出堆顶，然后看这个弹出的堆顶元素来自于哪个子大根堆，从该子大根堆中弹出当前的堆顶，入综合大根堆；
...
假设40亿数据，只有1G内存，找出频次最高的数

40亿数据全都放到内存是不行的，先准备100个空文件，对40亿数据挨个hash，然后对100取宇放到对应的文件中，最后对100个文件依次求频次最高的，汇总100个频次最高的结果，取最高的为结果

hash函数能保证数据被均匀映射，那么所有数据会被几乎均分到100个文件中，且相同的数肯定出现在一个文件中，因为相同的数算出来的hash总是相同的，那么只需对每个文件单独求频次即为其中包含数字的最高频次
...
*   深度优先遍历
    ...
    选取起点，遍历与其直接相邻的节点，每次遍历到某个节点时，优先遍历完与该节点相邻的其他节点，直至所有邻接节点都已经被遍历过

    思想是优先将一个节点遍历到低

*   广度优先遍历

    选取起点，遍历与其直接相邻的节点，只有当将这些直接相邻的节点被遍历完成后，再依次遍历间接相邻的节点，直至所有邻接节点都已经被遍历过
...
在使用hash取模作为分布式存储分配方案时，会造成增减机器时数据的全量迁移

为了减少数据迁移量，可以使用一致性hash方案，即hash环，每个数据节点随机一个环上位置，负责逆时针到前一个节点之间的数据

一致性hash存在问题:

1.  在数据节点少时，并不能保证每个数据节点随机获取到的位置是均分hash环
2.  增加节点时，立马造成hash环分配不均

使用虚拟节点解决hash环的问题：

​	每个数据节点随机获取大量环上位置，比如都是1000, 由于随机量大，hash能够呈现分布均匀的结果，这所有位置为虚拟节点，全部落到环上，一个数据节点拥有大量虚拟节点，且每个数据节点的虚拟节点在环上的分布是混杂且均匀的，此时解决问题一

​	增加节点时，只需要随机从每个原来的数据节点中抽出一部分，组成新数据节点的虚拟节点即可，且增加后，每个数据节点持有的虚拟节点仍然呈现均匀分布，解决问题二

​	虚拟节点自带可控负载均衡功能，只要给数据节点分配更多的虚拟节点，该数据节点将承受更多的流量
